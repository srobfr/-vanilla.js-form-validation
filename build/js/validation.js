...
function validateNames(firstName, lastName) {
    // checks if name fields are not empty and dont contain digits
    var containsNumbers = function containsNumbers(nameToTest) {
...
function validateTextArea1(firstTextBox) {
    // checks if first text field is not empty and shorter than 10 signs
...
function validateTextArea2(secondTextBox) {
    // checks if second text field is not empty and longer than 20 signs
...
function validateEmail(mail) {
    // checks if mail has no whitespaces, isnt empty, checks if has asd@asd.xy format
    var simpleMailCheck = function simpleMailCheck(mail) {
        return !/^.+@.+\..{2,6}$/.test(mail);
    };
    var whitespaceCheck = function whitespaceCheck(mail) {
        return (/\s/.test(mail)
        );
    };
...
function validatePass(password) {
    // checks if password has at least one letter, digit and special sign and is at least 8 characters long
    var hasAnyLetter = function hasAnyLetter(stringToTest) {
        return !/[a-z]+/ig.test(stringToTest);
    };
    var hasAnyNumber = function hasAnyNumber(stringToTest) {
        return !/[\d]+/.test(stringToTest);
    };
    var hasAnySpecial = function hasAnySpecial(stringToTest) {
...
function validateVID(vidNumber) {
    // checks if vid contains only between 1 and 5 digits and isn't empty
    var checkVID = function checkVID(number) {
...
function validateAmount(ticketsAmount) {
    // checks if ticket amount field isn't empty, value is less than 20 and has only digits
    var checkIfNumbers = function checkIfNumbers(number) {
...
function saveToLocalStorage() {
    var formArray = ['first_name', 'last_name', 'textarea_1', 'textarea_2', 'email', 'password', 'vid_number', 'tickets_count'];
    for (var x = 0; x < formArray.length; x++) {
        // Gets every field from form and saves it as a pair field:value
